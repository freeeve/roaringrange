...
## Outcome (DONE -- all 11 items)

No format/output-byte changes for valid inputs; goldens + conformance green. Only invalid inputs newly error.

### Docs (1-5)
1. `roaringrange.go` -- `ErrCompressedRecord` doc + message reworded: it now says the store was opened without a dictionary and points at `OpenRecordStoreWithDict` (Go DOES support zstd since task 049).
2. `transcode.go` -- comment now names the real function `parseEntries` (was `parseSplitEntries`).
3. `build.rs` -- the "Writes the v3 RRS index" doc block moved from `u32_len` (which had its own doc) onto the previously-undocumented `write_index`.
4. `wasm.rs` -- the `filtered_ids` doc moved off `FACET_COUNTS_TOP_PER_FIELD` (which kept its own) onto `filtered_ids`.
5. `hotcache.rs` -- module + `open` docs corrected from "one GET" to the accurate "two ranged reads (a tiny header, then the exact remaining bytes)"; kept the code (the body size is only known after the header, so the two-read shape is intrinsic).

### Error taxonomy + correctness (6-8)
6. `secondary.rs` -- a perm store missing the `"primary"` column is now `Malformed` (mismatched artifact), not `BadQuery`; the `rejects_perm_without_primary_column` test updated to match.
7. `index.rs` -- `Cursor::next`/`page` use `saturating_add` on the JS-supplied `pos+n` / `offset+limit` (was an unchecked usize add: debug panic / release wrap to a short page).
8. `monolithbuild.go` `WriteIndex` **and** `build.rs` `write_index` now reject duplicate n-gram keys (a dup made the sorted byte order tie-break-dependent -- breaking the byte-for-byte guarantee -- and the dict binary search ambiguous). Added to both for parity, with a test each.

### Writer-side length truncations (9)
Added range checks (error instead of silent truncation) to match the siblings that already check:
- Go: `terms.go` tail-posting u32 size, `sortcols.go` column-count u16 + column-name u16 + name-blob u32, `hotcache.go` string-blob u32 offset.
- Rust twins: `build.rs` facet `push` closure (name u16 + blob-offset u32, now returns `io::Result`), `write_sortcols` (column-count u16 + name u16 + blob u32).

### API consistency (10)
`wasm.rs` -- `RrsSecondaryIndex::searchCursorFiltered` and `RrssIndex::searchFiltered` now parse filters with `filter_sels` (accepting the same `[field,category]`-array OR `{field,category,exclude?}`-object shape the primary `RrsIndex` accepts) instead of the legacy array-only `filter_pairs`. Since neither the secondary nor the split core path supports excludes, an `exclude: true` selection now throws a clear error rather than being silently dropped.

### Style (11)
Renamed the `cap`-builtin-shadowing locals: `newDictBlockWriter(blockCap int)` in `terms.go`, and `splitCap := capFor(...)` in `splitsetbuild.go` / `termsplitsetbuild.go`.

### Tests
- `build.rs::write_index_rejects_duplicate_keys`, `monolithbuild_test.go::TestWriteIndexRejectsDuplicateKeys` (new) -- dup-key guard.
- `secondary.rs::rejects_perm_without_primary_column` -- updated to assert `Malformed`.

### Verification
- Rust: `cargo test --lib` matrix green -- default (95), splits+terms+vector+hotcache (203), zstd (95). `cargo fmt --check` clean; `cargo clippy --all-targets --features "splits terms vector hotcache"` clean; wasm feature compiles. `cargo doc` items 3-4 render with no new warnings (the 26 existing warnings are pre-existing intra-doc-link issues in `facet.rs`/`index.rs`, untouched here).
