...
`go/bm25.go` ports `quantize_impact` / `ImpactsAccumulator` / `write_impacts`,
byte-for-byte with Rust. `QuantizeImpact` does the math in float32 + round-half-
away-from-zero to match `f32::round`. Conformance via a shared golden
(`go/testdata/rrsb_build_golden.txt`, generated by the `gen_rrsb_golden` example),
asserted by BOTH `go/bm25_test.go` and Rust `bm25::tests::rrsb_golden_matches`. Go
+ Rust tests, gofmt -s, clippy all green. Original spec below.
